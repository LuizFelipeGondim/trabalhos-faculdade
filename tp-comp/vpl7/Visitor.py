...
class CtrGenVisitor(Visitor):
  ...
  def visit_eql(self, exp, type_var):
    ...
    """
    t_left = self.fresh_type_var()
    t_right = self.fresh_type_var()
    c_left = exp.left.accept(self, t_left)
    c_right = exp.right.accept(self, t_right)
    return {(type(True), type_var), (t_left, t_right)}.union(c_left).union(c_right)
  ...
  def visit_and(self, exp, type_var):
    ...
    c_left = exp.left.accept(self, t_left)
    c_right = exp.right.accept(self, t_right)
    return {(type(True), type_var), (type(True), t_left), (type(True), t_right)}.union(c_left).union(c_right)
  ...
  def visit_add(self, exp, type_var):
    ...
    c_left = exp.left.accept(self, t_left)
    c_right = exp.right.accept(self, t_right)
    return {(type(1), type_var), (type(1), t_left), (type(1), t_right)}.union(c_left).union(c_right)
  ...
  def visit_leq(self, exp, type_var):
    ...
    c_left = exp.left.accept(self, t_left)
    c_right = exp.right.accept(self, t_right)
    return {(type(True), type_var), (type(1), t_left), (type(1), t_right)}.union(c_left).union(c_right)
  ...
  def visit_neg(self, exp, type_var):
    ...
    Example:
      ...
      ["(<class 'int'>, 'TV_1')", "(<class 'int'>, <class 'int'>)"]
    """
    t_sub = self.fresh_type_var()
    c_sub = exp.exp.accept(self, t_sub)
    return {(type(1), type_var), (type(1), t_sub)}.union(c_sub)
  ...
  def visit_not(self, exp, type_var):
    ...
    t_sub = self.fresh_type_var()
    c_sub = exp.exp.accept(self, t_sub)
    return {(type(True), type_var), (type(True), t_sub)}.union(c_sub)
  ...
  def visit_let(self, exp, type_var):
    ...
    """
    t_def = self.fresh_type_var()
    t_body = self.fresh_type_var()
    c_def = exp.exp_def.accept(self, t_def)
    c_body = exp.exp_body.accept(self, t_body)
    return {(exp.identifier, t_def), (t_body, type_var)}.union(c_def).union(c_body)
  ...
  def visit_ifThenElse(self, exp, type_var):
    ...
    """
    t_cond = self.fresh_type_var()
    t_then = self.fresh_type_var()
    t_else = self.fresh_type_var()
    c_cond = exp.cond.accept(self, t_cond)
    c_then = exp.e0.accept(self, t_then)
    c_else = exp.e1.accept(self, t_else)
    return {(type(True), t_cond), (t_then, type_var), (t_else, type_var)}.union(c_cond).union(c_then).union(c_else)