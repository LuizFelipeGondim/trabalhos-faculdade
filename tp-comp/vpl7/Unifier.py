...
def unify(constraints, sets):
  ...
  for u, v in constraints:
    ...
    merged_set = sets[u].union(sets[v])

    for elem in merged_set:
      sets[elem] = merged_set
...
def name_sets(sets):
  ...
  * Ambiguous type: if any canonical type set contains more than one element.
  ...
  for var, t_set in sets.items():
    concrete_types = {t for t in t_set if isinstance(t, type)}
    ...
    canonical_type = next(iter(concrete_types))
    type_names[var] = canonical_type
